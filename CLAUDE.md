...
- **PoolParty pinned to `10.2.2` — do NOT bump to `10.3.0` (vendor defect).** `10.3.0` adds a startup migration `PoolPartyRoleMigrationService.migrateRealmToClientRoles` that throws `IllegalStateException: Cannot find client id for client ppgs` and the Tomcat context fails to start (pod stuck 0/1, startup probe 500s → crash loop). **Verified end-to-end that our side is correct**: the realm JSON has the `ppgs` client → the `KeycloakRealmImport` CR has it → the live realm has it → `poolparty_auth_admin` (PoolParty's exact admin account) resolves it, and PoolParty's *exact* query (`GET https://auth.<sub>.<base>/admin/realms/poolparty/clients?clientId=ppgs`, admin-cli password grant, from in-cluster) returns the client with full `view/configure/manage` access. PoolParty 10.3.0 still can't find it after a clean restart → it's a bug inside the 10.3.0 image, not our chart/realm/extractor/domain/Keycloak. Before re-bumping, get a fix from Graphwise (or a flag to skip `migrateRealmToClientRoles`) and re-test. The `10.2.2 → 10.3.0` bump shipped in 2.1.2 and was rolled back here. (Debug trail: this was a multi-hour chase that also surfaced + fixed the SSM AMI selection, the KIND-vs-host containerd skew revert, the docker-group self-heal, and the reset-helm/render-values `gw-pse.com` default — all separate commits.)
