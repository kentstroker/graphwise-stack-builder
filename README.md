...
It is a **demo / evaluation** environment — not production-ready (default passwords, single-replica services, no HA or hardening). A personal project by Kent Stroker — not a Graphwise product, and not affiliated with, endorsed by, or supported by Graphwise or Ontotext. Published MIT-licensed (AS-IS, no warranty, no support) so customers, partners, and the semantic-web community can reference it when standing up their own evaluation stacks.
