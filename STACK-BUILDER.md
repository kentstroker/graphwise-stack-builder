...
- [Appendix: `scripts/` reference](#appendix-scripts-reference)
  ...
  - [Licenses, secrets & realm](#licenses-secrets--realm)
  - [Preflight & validation](#preflight--validation)
  - [Day-2 lifecycle scripts](#day-2-lifecycle-scripts)
  - [Workflow database seed](#workflow-database-seed)
  - [Branding, images & utilities](#branding-images--utilities)
...
- **PoolParty 10.3.2** — Thesaurus, GraphSearch, Extractor REST
...
|---|---|---|
| **Provisioning & deploy** | | |
| `deploy-stack.sh` | EC2 | One-shot non-interactive build; chains bootstrap → realm/licenses → reset-helm → workflow restore |
| `cluster-bootstrap.sh` | EC2 | Install cluster operators + observability + wildcard cert; pre-load Refine/Keycloak images |
| `reset-helm.sh` | EC2 | Wipe (incl. PVCs) and reinstall both Helm releases from scratch |
| `render-values.sh` | EC2 *(auto)* | Emit the two per-subdomain Helm values overlays into `~/.graphwise-stack/` |
| **Licenses, secrets & realm** | | |
| `install-licenses.sh` | EC2 | Create the three vendor license Secrets from `files/licenses/` |
| `extract-poolparty-realm.sh` | EC2 | Pull + placeholder-substitute the PoolParty Keycloak realm JSON |
| **Preflight & validation** | | |
| `preflight-reset-helm.sh` | EC2 | Read-only gate that checks every precondition before the destructive reset-helm |
| `validate-bootstrap.sh` | EC2 | Post-bootstrap operator/issuer/secret health check |
| `validate-stack.sh` | EC2 | Post-reset-helm workload / cert / OIDC / HTTPS health check |
| **Day-2 lifecycle** | | |
