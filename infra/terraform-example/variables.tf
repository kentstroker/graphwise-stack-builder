...
variable "ami_override" {
  description = "Specific AMI ID to launch the instance with. When empty (default), the module resolves the latest default AL2023 ARM64 AMI via the SSM parameter data.aws_ssm_parameter.al2023_arm64 (kernel-default line) -- correct for first apply, but note that an AWS AMI refresh would otherwise mark the EC2 for force-replace (destroying all data on the root EBS volume). After the first successful apply, capture `terraform output -raw ami_id` and set this variable to the resulting `ami-...` ID, then re-run `terraform plan` -- you should see no changes. From then on the module ignores AWS AMI publishes entirely. To upgrade later, look up the desired AMI (e.g. `aws ssm get-parameter --name /aws/service/ami-amazon-linux-latest/al2023-ami-kernel-default-arm64 --query Parameter.Value --output text`) and bump this value (will trigger a controlled replace; snapshot EBS first if you care). Belt-and-braces: aws_instance.stack also ignores ami changes via lifecycle.ignore_changes, so even an unlocked deployment is protected once provisioned."
  type        = string
  default     = ""
